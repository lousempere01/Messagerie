...
void *broadcast(void *dS_client) {
  ...
  while (1) {
    ...
    i = 0;
    while (i < MAX_CLIENT && tab_client[i] != -1) {
      if (tab_client[i] != dSC) {
...
void *connexion(void *dServeur) {
  ...
  while (1) {
    ...
    for (int i = 0; i < MAX_CLIENT; i++) {
      if (tab_client[i] == -1) { // on accepte les nouveaux clients dans des cases non occupés
...
int main(int argc, char *argv[]) {
  if (argc != 2) {
    printf(
        "Error: You must provide exactly 1 argument.\nUsage: ./serv <port>\n");
    exit(EXIT_FAILURE);
  }
