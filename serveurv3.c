...
void *broadcast(void *client) {
    ...
    while (1) {
        ...
        if (nb_recv == -1) {
            ...
            break;
        } else if (strcmp(message.contenu, "fin") == 0) { // Si le client envoie "fin", arrêter la discussion pour ce client
            printf("Fin de la discussion pour client : %s\n", data->pseudo);
            break;
        } else if (strncmp(message.contenu, "/mp", 3) == 0) { //Si le client envoie la commande /mp pour envoyer un message privé
        ...
        } else{ // Envoyer le message aux autres clients
...
int isPseudoUsed(char pseudo[]) {
    for (int i = 0; i < MAX_CLIENT; i++) {
        if (tab_client[i].socket != -1 && strcmp(tab_client[i].pseudo, pseudo) == 0) {
            return 1; // Le pseudo est déjà utilisé
        }
    }
    return 0; // Le pseudo est disponible
}
...
void *connexion(void *dServeur) {
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_CLIENT; i++) {
            if (tab_client[i].socket == -1) { // on accepte les nouveaux clients dans des cases non occupés
                ...
                if (isPseudoUsed(tab_client[i].pseudo)) {
                    char erreur_message[BUFFER_SIZE];
                    snprintf(erreur_message, BUFFER_SIZE, "Le pseudo '%s' est déjà utilisé. Veuillez choisir un autre pseudo.", tab_client[i].pseudo);
                    send(tab_client[i].socket, erreur_message, strlen(erreur_message), 0);
                    close(tab_client[i].socket);
                    tab_client[i].socket = -1;
